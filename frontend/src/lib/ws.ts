/**
 * Cliente mínimo del hub WebSocket (fase 12, ``/api/v1/ws``): auth →
 * subscribe → eventos. El plano de mesas lo usa como AVISO de invalidación
 * (§5.4): un evento en «sales» o «restaurant» ⇒ refrescar el plano; nunca
 * transporta estado de negocio.
 *
...
export function connectHub(options: HubOptions): HubConnection {
  ...
  // Suscribirse en cuanto el servidor lo permita (tras ready+auth).
  open();
  // Nota: «subscribed» marca el live; «connecting» ya se reportó al abrir.
